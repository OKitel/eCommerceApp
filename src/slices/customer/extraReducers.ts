...
export function reducerChangePasswordFulfilled(
  state: Draft<TCustomerSliceState>,
  action: PayloadAction<Customer | undefined>,
): void {
  state.progress.update = false;
...
export function reducerChangePasswordRejected(state: Draft<TCustomerSliceState>, action: PayloadAction<unknown>): void {
  const { payload } = action;
